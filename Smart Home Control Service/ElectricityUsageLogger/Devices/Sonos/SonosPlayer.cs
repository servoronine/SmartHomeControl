...
namespace SmartHomeControl.Devices.Sonos {
	public class SonosPlayer
	{
		...
		private SonosPlayerState currentState = new SonosPlayerState();
		...
		private void ParseChangeXML(string newState)
		{
			...
			var preliminaryState = new SonosPlayerState
				                       {
		...
		public SonosPlayerState CurrentState
		{
		...
		private void UpdateState(object state)
		{
            ...
            try {
                CurrentState.MuteStatus = GetMute();
                CurrentState.Volume = GetVolume();
            } catch {
                //void
            }
        ...
        public void Mute() {
            ...
            arguments[0] = new UPnPArgument("InstanceID", 0u);
            arguments[1] = new UPnPArgument("Channel", "Master");
            arguments[2] = new UPnPArgument("DesiredMute", true);
        ...
        private ushort GetVolume() {
            var arguments = new UPnPArgument[3];
            arguments[0] = new UPnPArgument("InstanceID", 0u);
            arguments[1] = new UPnPArgument("Channel", "Master");
            arguments[2] = new UPnPArgument("CurrentVolume", (ushort)0);
		...
		public virtual SonosSearchResult GetArtists(string objectId = null, uint startIndex = 0, uint requestedCount = 100)
		{
			if (objectId == null)
				objectId = "A:ARTIST";
		...
		private SonosSearchResult Browse(string action, uint startIndex = 0u, uint requestedCount = 100u)
		{
			var arguments = new UPnPArgument[10];
